    ...
    if(get_option('enable_like_box')){
      ...
      <?php endif;//END FB
        if($url_insta): //facebook?>
          <div><a class="like_insta" href="<?php echo $url_insta; ?>" target="blank" rel="nofollow"><i class="fa fa-instagram"></i> フォローする</a></div>
      <?php endif;//END FB
      echo '</div></div></div>';
    }
  }//end function
  ...
  function output_sng_related_posts() {
    ...
    if($related_posts) {
      ...
      foreach($related_posts as $related_post):
		  ...
		  $featured_image_id = get_post_thumbnail_id( $related_post->ID );
